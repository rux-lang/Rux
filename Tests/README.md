...
All repository tests live below this directory. Executable tests have six categories, with reusable source providers under `Fixtures/`:
...
| `Fixtures/`                  | Local source-library providers consumed by executable tests      |
...
Runtime-output regressions use checked native writes followed by immediate process exit, covering binary bytes and output larger than pipe capacity. The CLI report case runs in Debug and Release with one and four workers alongside independent captures, checking empty, multiline, and large Unicode panic messages. A Windows x86-64 writer shim deliberately overwrites its incoming stack argument after each call, ensuring every subsequent write supplies its arguments again. `rux --manifest Tests/Fixtures/RuntimeFailure/Fixture.toml build --all` covers emission and linking on every target; executing that fixture deliberately traps after writing its panic message.
