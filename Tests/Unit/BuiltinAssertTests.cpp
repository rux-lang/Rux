...
TEST_CASE("assertion intrinsics require declarations and enforce their signatures") {
    ...
    CHECK(std::ranges::any_of(signatureDiagnostics, [](const SemanticDiagnostic &diagnostic) {
        return diagnostic.message == "argument 1 to 'Assert' has type 'char8[..]', but parameter 'condition' "
                                     "requires 'bool8'";
    }));
    CHECK(std::ranges::any_of(signatureDiagnostics, [](const SemanticDiagnostic &diagnostic) {
        return diagnostic.message ==
               "argument 2 to 'DebugAssert' has type 'int', but parameter 'message' requires 'char8[..]'";
    }));
}
