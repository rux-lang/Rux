...
TEST_CASE("A string prefix is part of the literal's spelling") {
    const auto result = Lex(R"(let a = "plain"; let b = c8"eight"; let c = c16"sixteen"; let d = c32"thirty-two";)");
    REQUIRE(result.diagnostics.empty());
    ...
    CHECK_EQ(literals[1], R"(c8"eight")");
    CHECK_EQ(literals[2], R"(c16"sixteen")");
    CHECK_EQ(literals[3], R"(c32"thirty-two")");
}

TEST_CASE("Escapes inside a prefixed string are scanned like any other") {
    const auto result = Lex(R"(let a = c16"a\tb\u{1F680}\"";)");
    REQUIRE(result.diagnostics.empty());
    REQUIRE_GE(result.tokens.size(), 4);
    CHECK(result.tokens[3].Is(TokenKind::StringLiteral));
    CHECK_EQ(result.tokens[3].text, R"(c16"a\tb\u{1F680}\"")");
}

TEST_CASE("A string prefix needs its quote to be a prefix at all") {
    // Only the exact spelling is a prefix: the identifier `c8`, an identifier that merely starts with one, and a
    // prefix separated from its quote are all ordinary identifiers.
    const auto result = Lex(R"(c8 c8x c16foo c16 "spaced")");
    REQUIRE(result.diagnostics.empty());
    REQUIRE_EQ(result.tokens.size(), 6);
    CHECK(result.tokens[0].Is(TokenKind::Ident));
    CHECK_EQ(result.tokens[0].text, "c8");
    CHECK(result.tokens[1].Is(TokenKind::Ident));
    CHECK_EQ(result.tokens[1].text, "c8x");
    CHECK(result.tokens[2].Is(TokenKind::Ident));
    CHECK_EQ(result.tokens[2].text, "c16foo");
    CHECK(result.tokens[3].Is(TokenKind::Ident));
    CHECK_EQ(result.tokens[3].text, "c16");
