...
TEST_CASE("unary diagnostics name the operator and required operand family") {
    ...
    CHECK_EQ(diagnostics[1].message, "operator '-' requires a numeric operand, but found 'char8[..]'");
    ...
    CHECK_EQ(diagnostics[4].message, "operator '++' requires a numeric operand, but found 'char8[..]'");
}
...
TEST_CASE("binary diagnostics identify left and right operands across every operator family") {
    ...
    CHECK_EQ(diagnostics[0].message, "operator '*' cannot combine left operand 'int' with right operand 'char8[..]'");
    ...
    CHECK_EQ(diagnostics[8].message, "operator '<' cannot compare left operand 'int' with right operand 'char8[..]'");
}
...
TEST_CASE("an indexed assignment checks its value and index against the setter it resolved") {
    ...
    REQUIRE_EQ(diagnostics.size(), 2);
    CHECK_EQ(diagnostics[0].message, "cannot pass 'char8[..]' to parameter of type 'int'");
    CHECK_EQ(diagnostics[1].message, "no '[]=' on 'Vect' accepts an index of type 'char8[..]'");
}
