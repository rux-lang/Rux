...
bool TypeRef::IsNumeric() const noexcept {
    switch (kind) {
        case Kind::Int8:  case Kind::Int16:  case Kind::Int32:  case Kind::Int64:
        case Kind::UInt8: case Kind::UInt16: case Kind::UInt32: case Kind::UInt64:
...
private:
    ...
    void CollectDecl(const Decl& decl, Scope& scope) {
        ...
        auto simple = [&](Symbol::Kind kind, const std::string& name,
            ...
            if (scope.Define(sym, diags_, currentFile_) && isGlobal) {
                symbols_.push_back({pubKind, name, currentFile_,
        ...
        if (auto* d = dynamic_cast<const FuncDecl*>(&decl))
            ...
            if (scope.Define(sym, diags_, currentFile_) && isGlobal)
                symbols_.push_back({SemaSymbol::Kind::Interface, d->name,
            ...
            sym.isMut = true;
            if (scope.Define(sym, diags_, currentFile_) && isGlobal)
                symbols_.push_back({SemaSymbol::Kind::Var, d->name,
    ...
    TypeRef CheckBinary(TokenKind op, const TypeRef& l, const TypeRef& r, SourceLocation loc) {
        ...
        switch (op) {
            ...
            case TK::Slash: case TK::Percent: case TK::StarStar:
                if (!l.IsNumeric())
                    EmitError(loc, std::format("'{}' applied to non-numeric type '{}'",
                        op == TK::Plus ? "+" : op == TK::Minus ? "-" :
                        op == TK::Star ? "*" : op == TK::Slash ? "/" :
                        op == TK::Percent ? "%" : "**", l.ToString()));
                return l;

            case TK::Amp: case TK::Pipe: case TK::Caret:
            case TK::LessLess: case TK::GreaterGreater:
                if (!l.IsInteger())
                    EmitError(loc, std::format("bitwise operator applied to non-integer type '{}'",
                        l.ToString()));
                return l;

            case TK::AmpAmp: case TK::PipePipe:
                if (!l.IsBool())
            ...
            case TK::Equal: case TK::BangEqual:
            case TK::Less:  case TK::LessEqual:
            case TK::Greater: case TK::GreaterEqual:
                return TypeRef::MakeBool();
