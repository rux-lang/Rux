    ...
    namespace {
        ...
        private:
            ...
            void EmitPhiMoves(const uint32_t from, uint32_t to) {
                ...
                struct PCopy { LirReg src, dst; TypeRef type; };
                std::vector<PCopy> pending;
                for (const auto& m : it2->second) {
                    if (m.src == m.dst) continue;
                    if (!slotMap.contains(m.src) || !slotMap.contains(m.dst)) continue;
                ...
                while (progress) {
                    ...
                    for (std::size_t i = 0; i < pending.size();) {
                        ...
                        for (std::size_t j = 0; j < pending.size(); ++j)
                            if (j != i && pending[j].src == dst) { needed = true; break; }
                        if (!needed) {
                ...
                while (!pending.empty()) {
                    ...
                    if (wide) {
                        ...
                        enc.SubRspImm8(8);
                        if (f32) enc.MovssXmm0StoreRspNoDisp();
                        else     enc.MovsdXmm0StoreRspNoDisp();
                    } else {
                        enc.PushRax();
                    }
                    ...
                    if (wide) {
                        ...
                        if (f32) enc.MovssXmm0LoadRspNoDisp();
                        else     enc.MovsdXmm0LoadRspNoDisp();
                        enc.AddRspImm8(8);
                    } else {
                        enc.PopRax();
                    }
