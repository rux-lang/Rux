...
static void WriteU16(Buf& b, uint16_t v) { b.push_back(v & 0xFF); b.push_back(v >> 8); }
...
static void WriteCStr (Buf& b, const char* s) { while (*s) b.push_back(*s++); b.push_back(0); }
...
bool Linker::Link(const std::filesystem::path& outputPath) {
    ...
    struct ObjLayout { uint32_t textOff, rodataOff, dataOff; };
    ...
    if (!out) { Error("cannot open output file: " + outputPath.string()); return false; }
    ...
    const auto wDir = [&](uint32_t rva, uint32_t sz) { wU32(rva); wU32(sz); };
    ...
    wU32(kSecAlign);
    wU32(kFileAlign);
    wU16(6); wU16(0);          // MajorOSVersion / MinorOSVersion
    wU16(0); wU16(0);          // MajorImageVersion / MinorImageVersion
    wU16(6); wU16(0);          // MajorSubsystemVersion 6.0 (Vista+)
    ...
    wDir(0, 0); wDir(0, 0); wDir(0, 0); wDir(0, 0); wDir(0, 0); wDir(0, 0);      // [2..7]
    wDir(0, 0); wDir(0, 0); wDir(0, 0); wDir(0, 0);                               // [8..11]
    wDir(rdataRva + iatOff, static_cast<uint32_t>((numImports + 1) * 8));         // [12] IAT
    wDir(0, 0); wDir(0, 0); wDir(0, 0);                                           // [13..15]

    // Section Headers (40 bytes each)
    wSec8(".text");
    wU32(textVirtSize); wU32(textRva); wU32(textFileSize); wU32(textFileOff);
    wU32(0); wU32(0); wU16(0); wU16(0);
    wU32(kScnText);

    wSec8(".rdata");
    wU32(rdataVirtSize); wU32(rdataRva); wU32(rdataFileSize); wU32(rdataFileOff);
    wU32(0); wU32(0); wU16(0); wU16(0);
    wU32(kScnRData);

    if (!mergedData.empty()) {
        wSec8(".data");
        wU32(dataVirtSize); wU32(dataRva); wU32(dataFileSize); wU32(dataFileOff);
        wU32(0); wU32(0); wU16(0); wU16(0);
        wU32(kScnData);
    }
    ...
    // Section data
    wBuf(textBuf);  padTo(kFileAlign);
    wBuf(rdataBuf); padTo(kFileAlign);
    if (!mergedData.empty()) { wBuf(mergedData); padTo(kFileAlign); }
