...
namespace Rux {
    ...
    namespace {
        ...
        constexpr std::size_t MaxSize(const Range& range, Proj proj = {}) {
            ...
            for (const auto& item : range) {
                ...
                maxSize = std::max(static_cast<std::size_t>(std::size(value)),
                                   maxSize);
            }
        ...
        std::size_t GetTerminalWidth() {
            ...
            if (GetConsoleScreenBufferInfo(GetStdHandle(STD_OUTPUT_HANDLE),
                ...
                return std::max(static_cast<std::size_t>(width),
                                Layout::MinTerminalWidth);
            }
#else
            winsize w{};
            if (ioctl(STDOUT_FILENO, TIOCGWINSZ, &w) != -1 && w.ws_col > 0) {
                return std::max(static_cast<std::size_t>(w.ws_col),
                                Layout::MinTerminalWidth);
            }
#endif
        ...
        constexpr std::size_t UsableWidth(const std::size_t terminalWidth,
                                          const std::size_t indent) {
            if (terminalWidth <= indent) {
                return Layout::MinDescriptionWidth;
            }

            return std::max(terminalWidth - indent,
                            Layout::MinDescriptionWidth);
        }

        template <typename Callback>
        constexpr void ProcessLine(std::string_view line,
                                   const std::size_t width,
                                   Callback callback) {
            while (!line.empty()) {
                // Skip leading spaces by sliding the view forward
                const auto firstNonSpace =
                    line.find_first_not_of(Layout::Whitespace);
                ...
                if (width < line.size()) {
                    ...
                    if (cutLength == std::string_view::npos) {
                        cutLength =
                            line.find_first_of(Layout::Whitespace, width);
                        if (cutLength == std::string_view::npos) {
                            cutLength = line.size();
                        }
                    }
                ...
                const auto lastNonSpace =
                    piece.find_last_not_of(Layout::Whitespace);
        ...
        template <typename Callback>
        constexpr void Wrap(std::string_view text,
                            const std::size_t width,
                            Callback callback) {
            while (!text.empty()) {
                ...
                if (const std::string_view line = text.substr(0, newLinePos);
                    line.empty()) {
                    callback(""sv);
                }
                else {
        ...
        void PrintBlock(std::string_view title,
            ...
            Wrap(text, usable, [&](std::string_view line) -> void {
                ...
                // newline)
                std::println(
                    "{0:<{1}}{2}", "", line.empty() ? 0 : indent, line);
            });
        ...
        void PrintAligned(const std::string_view left,
                          ...
                          const size_t termWidth) {
            const size_t indent =
                Layout::BlockIndent + leftWidth + Layout::AlignedPadding;
            const size_t width = UsableWidth(termWidth, indent);
            bool first = true;

            Wrap(right, width, [&](std::string_view line) {
                if (first) {
                    std::println("{0:<{1}}{2:<{3}}{4}",
                                 "",
                                 Layout::BlockIndent,
                                 left,
                                 leftWidth + Layout::AlignedPadding,
                                 line);
        ...
        namespace Data {
            // Add
            constexpr std::array add_usage = {"[package]"sv,
                                              "[package]@[version]"sv,
                                              "[package] --path [path]"sv};
            constexpr std::array add_opts = {
                OptionDoc{.flags = "--path <path>"sv,
                          .desc = "Add a local path-based dependency"sv}};
            constexpr std::array add_exs = {
                "Std"sv, "Std@0.1.0"sv, "Json --path ../Json"sv};
            ...
            constexpr std::array build_opts = {
                OptionDoc{
                    .flags = "--debug"sv,
                    .desc = "Build with debug symbols (unoptimized output)"sv},
                OptionDoc{
                    .flags = "--profile <n>"sv,
                    .desc =
                        "Build using a custom profile defined in Rux.toml"sv},
                OptionDoc{
                    .flags = "--release"sv,
                    .desc =
                        "Build with release profile (optimized, no debug info)"sv},
                OptionDoc{
                    .flags = "--stats"sv,
                    .desc =
                        "Print build timing, source, performance, and output statistics"sv},
                OptionDoc{
                    .flags = "--target <triple>"sv,
                    .desc =
                        "Build for the specified target platform (e.g. x86, x64)"sv},
                OptionDoc{
                    .flags = "-q, --quiet"sv,
                    .desc =
                        "Suppress non-essential output (only errors are shown)"sv},
                OptionDoc{
                    .flags = "-v, --verbose"sv,
                    .desc =
                        "Enable verbose output for detailed build information"sv},
                OptionDoc{.flags = "--dump-asm"sv,
                          .desc =
                              "Write x86-64 assembly to Temp/Asm/out.asm"sv},
                OptionDoc{.flags = "--dump-ast"sv,
                          .desc =
                              "Write the parsed AST to Temp/Ast/<file>.ast"sv},
                OptionDoc{.flags = "--dump-hir"sv,
                          .desc =
                              "Write the high-level IR to Temp/Hir/hir.txt"sv},
                OptionDoc{.flags = "--dump-lir"sv,
                          .desc =
                              "Write the low-level IR to Temp/Lir/lir.txt"sv},
                OptionDoc{
                    .flags = "--dump-rcu"sv,
                    .desc =
                        "Write RCU object files to Temp/Obj/ and text dumps to Temp/Rcu/"sv},
                OptionDoc{
                    .flags = "--dump-sema"sv,
                    .desc =
                        "Write semantic analysis results to Temp/Sema/sema.txt"sv},
                OptionDoc{
                    .flags = "--dump-tokens"sv,
                    .desc =
                        "Write the token stream to Temp/Tokens/<file>.tokens"sv}};
            ...
            constexpr std::array check_opts = {
                OptionDoc{.flags = "--json"sv,
                          .desc = "Output diagnostics in JSON format"sv},
                OptionDoc{
                    .flags = "--target <triple>"sv,
                    .desc =
                        "Check code health for a specific target platform"sv}};
            constexpr std::array check_exs = {
                ""sv, "--json"sv, "--target windows-x64"sv};

            // Clean
            constexpr std::array clean_usage = {"[options]"sv};
            constexpr std::array clean_opts = {OptionDoc{
                .flags = "--temp"sv,
                .desc = "Remove only the temporary build directory (Temp/)"sv}};
            ...
            constexpr std::array doc_opts = {OptionDoc{
                .flags = "--open"sv,
                .desc =
                    "Open the generated documentation index in a browser"sv}};
            ...
            constexpr std::array fmt_opts = {
                OptionDoc{
                    .flags = "--check"sv,
                    .desc =
                        "Check file formatting status without modifying source files"sv},
                OptionDoc{
                    .flags = "--manifest"sv,
                    .desc =
                        "Format only the manifest configuration file (Rux.toml)"sv}};
            ...
            constexpr std::array info_opts = {
                OptionDoc{.flags = "--json"sv,
                          .desc = "Output package metadata in JSON format"sv}};
            ...
            constexpr std::array init_opts = {
                OptionDoc{
                    .flags = "--bin"sv,
                    .desc =
                        "Create an executable binary package configuration"sv},
                OptionDoc{.flags = "--lib"sv,
                          .desc = "Create a library package configuration"sv}};
            constexpr std::array init_exs = {""sv, "--bin"sv};

            // Install
            constexpr std::array install_usage = {""sv,
                                                  "[package]"sv,
                                                  "[package]@[version]"sv,
                                                  "--dev [package]"sv};
            constexpr std::array install_opts = {OptionDoc{
                .flags = "--dev"sv,
                .desc = "Clone the package repository's development branch"sv}};
            constexpr std::array install_exs = {
                ""sv, "Std"sv, "Std@0.1.0"sv, "--dev Std"sv, "--dev Windows"sv};
            ...
            constexpr std::array list_opts = {OptionDoc{
                .flags = "--global"sv,
                .desc =
                    "List packages in the global environment cache instead of the local manifest"sv}};
            ...
            constexpr std::array new_opts = {
                OptionDoc{
                    .flags = "--bin"sv,
                    .desc =
                        "Create an executable binary package application (default)"sv},
                OptionDoc{.flags = "--lib"sv,
                          .desc = "Create a library code package"sv},
                OptionDoc{
                    .flags = "--path <dir>"sv,
                    .desc = "Create the workspace in a specific directory"sv}};
            ...
            constexpr std::array run_opts = {
                OptionDoc{.flags = "--release"sv,
                          .desc = "Build with release profile"sv}};
            constexpr std::array run_exs = {
                ""sv, "--release"sv, "-- --port 8080"sv};
            ...
            constexpr std::array update_opts = {OptionDoc{
                .flags = "--global"sv,
                .desc =
                    "Update all entries in the global environment cache"sv}};
            constexpr std::array update_exs = {""sv, "--global"sv};

            // Version
            constexpr std::array version_exs = {
                ""sv, "rux -V"sv, "rux --version"sv};
        } // namespace Data

        namespace GlobalOpts {
            constexpr std::array catalog = {
                OptionDoc{.flags = "--color <auto|on|off>"sv,
                          .desc = "Control colored console output"sv},
                OptionDoc{.flags = "-h, --help"sv,
                          .desc = "Show help information"sv},
                OptionDoc{.flags = "-q, --quiet"sv,
                          .desc = "Do not show log messages"sv},
                OptionDoc{.flags = "-v, --verbose"sv,
                          .desc = "Use verbose output"sv},
                OptionDoc{.flags = "-V, --version"sv,
                          .desc = "Show version information"sv}};
        } // namespace GlobalOpts
        ...
        constexpr std::array G_COMMAND_HELP_MAPS = {
            ...
            CommandDoc{
                .name = "check"sv,
                .shortDesc =
                    "Check package source code for errors without building"sv,
                .description =
            ...
            CommandDoc{
                ...
                .postUsage =
                    "Removes the configured build output directory and Temp/ folder."sv,
                .footer = {},
            ...
            CommandDoc{
                ...
                .description =
                    "Format all *.rux source files and *.toml manifests"sv,
            ...
            CommandDoc{.name = "info"sv,
                       .shortDesc =
                           "Show package metadata and manifest information"sv,
                       .description =
                           "Show information about an installed Rux package"sv,
            ...
            CommandDoc{
                .name = "init"sv,
                .shortDesc =
                    "Initialize a Rux package in the current directory"sv,
                .description = {},
            ...
            CommandDoc{.name = "new"sv,
                       ...
                       .description =
                           "Create a new Rux package in a new directory"sv,
            ...
            CommandDoc{
                ...
                .postUsage =
                    "Arguments after '--' are forwarded to the executable."sv,
                .footer = {},
            ...
            CommandDoc{
                ...
                .postUsage =
                    "Without a package name, removes all registry dependencies "
            ...
            CommandDoc{.name = "version"sv,
                       ...
                       .description =
                           "Show information about the Rux toolchain version"sv,
                       .usage = {},
                       .postUsage = {},
                       .footer = {},
        ...
        constexpr auto VerifyRegistryIntegrity() -> bool {
            if constexpr (constexpr auto dup = std::ranges::adjacent_find(
                              G_COMMAND_HELP_MAPS, {}, &CommandDoc::name);
                          dup != G_COMMAND_HELP_MAPS.end()) {
                return false;
            }

            return std::ranges::all_of(
                G_COMMAND_HELP_MAPS, [](const CommandDoc& cmd) -> bool {
        ...
        static_assert(
            std::ranges::is_sorted(G_COMMAND_HELP_MAPS,
                                   std::less<>{},
                                   &CommandDoc::name),
        ...
        static_assert(VerifyRegistryIntegrity(),
                      "Duplicate commands, empty descriptions, or redundant "
                      "metadata detected!");

        constexpr auto G_CMD_WIDTH =
            MaxSize(G_COMMAND_HELP_MAPS, &CommandDoc::name);
        constexpr auto G_OPT_WIDTH =
            MaxSize(GlobalOpts::catalog, &OptionDoc::flags);
    } // namespace
    ...
    void Cli::PrintHelpFor(const std::string_view command) {
        ...
        const auto it = std::ranges::lower_bound(
            G_COMMAND_HELP_MAPS, command, std::less<>{}, &CommandDoc::name);
        ...
        const std::string_view longOrShortDescription =
            !description.empty() ? description : shortDesc;
        Wrap(longOrShortDescription,
             termWidth,
    ...
    void Cli::PrintVersion() {
        std::println(
            "Rux {} ({} {})", RUX_VERSION, RUX_BUILD_DATE, RUX_BUILD_TIME);
    }
    ...
    void Cli::PrintUnknownOption(std::string_view option,
