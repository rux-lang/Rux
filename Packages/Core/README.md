...
- **Interfaces for generic code** — `Drop` marks a type move-only so its value has one owner at a time (destruction is not emitted yet; see the interface's safety note), `Equatable` and `Comparable` give equality and a total order through `Self`-typed operands, `Hashable` gives the 64-bit summary a table keys on, and `Iterator`/`Iterable` name the iteration protocol the compiler drives by shape. All are written as bounds: `func Sort<T: Comparable>(...)` resolves `Self` to the type argument at each instantiation, with no dynamic dispatch.
