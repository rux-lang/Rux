...
`Copy` values; the package owns no storage and needs no constructors or destructors. Multi-result operations such
as `SinCos` return tuples. Checked operations use raw writable output pointers, for example
