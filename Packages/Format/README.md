...
- **The wide widths** — `Float80`, `Float128`, `Float256` and `Float512`, held as their bits. The language has no arithmetic at these widths yet, so a value is made from bits or read from text, answers `IsNan`, `IsInfinite`, `IsFinite` and `IsNegative`, and renders through `FormatWith` as the shortest decimal that reads back to exactly the same bits — found with exact big-integer arithmetic (`BigNat`), which allocates. None of the four implements `Display` or `Debug`: the protocols offer nowhere to pass an allocator, and implementing them anyway would mean a hidden allocation behind an ordinary `{}` and no way for a caller who has chosen an allocator to make this one follow. Where a rendering allocates, the caller says from where. `ParseFloat80` through `ParseFloat512` read decimal text to the nearest value with ties to even, and every rendering and reading is held to reference vectors the compiler's own exact float arithmetic produced independently.
