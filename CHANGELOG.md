...
- **Comparison operators a struct does not declare** — only the exact operator a type declared was dispatched, and the rest fell through to comparing the operands' raw representation, quietly answering a question the type never defined. A struct whose `==` ignored a field reported `a == b` and `a != b` both true. An operator is now derived from the ones the type does declare — `a != b` from `==`, and `a > b` from `<` with its operands reversed, each naming its operands exactly once so nothing is evaluated twice. A declared operator still wins over the derivation. Where nothing can be derived the comparison is rejected, naming the operator to declare, rather than compared by representation. `<=` and `>=` are not yet derived: `(a < b) || (a == b)` names both operands twice and lowering cannot yet bind one to a temporary, so they must be declared for now. Enums, primitives and tuples are unaffected.
