...
- **`intrinsic`** — marks a declaration the compiler supplies rather than Rux source: `intrinsic const CurrentTarget: Target;` and `intrinsic func Assert(condition: bool, message: char8[]);`. The declaration names the intrinsic, so nothing is written twice: a constant takes its type (`Target`), a free function its own name (`Assert`), and a method the type it extends (`extend Config { intrinsic func Get(...); }` is `Config.Get`). An intrinsic constant has no initializer and an intrinsic function has no body. This replaces the `#Intrinsic("Name")` attribute and the `$` sigil, which together marked the same thing three ways and had to be kept in agreement.
- **`when` — conditional compilation** — `when cond { ... } else when cond { ... } else { ... }` evaluates its condition while compiling and keeps only the taken branch; the others are never resolved or type-checked, so they may reference symbols that do not exist on the current build. `if` is the run-time conditional and `when` the compile-time one, and a chain keeps the keyword it opened with: `else if` inside a `when` chain is an error. Conditions are built from `const` declarations, enums, literals, imported compiler parameters such as `CurrentTarget.os`, and the usual operators (`!`, `&&`, `||`, comparisons, arithmetic). It works both between declarations and inside a function body, where it introduces no scope of its own — a binding it makes is visible to the statements that follow. A condition that is not a compile-time constant, or not a `bool`, is an error.
- **`CurrentTarget.os` and the `OperatingSystem` enum** — `CurrentTarget.os` is a compile-time value of the built-in `OperatingSystem` enum, so a build selects its platform code with `when CurrentTarget.os == OperatingSystem::Windows { ... }`. Each variant is a system, not a family: `Windows`, `Linux`, `MacOS`, `FreeBSD`, `OpenBSD`, `NetBSD`, `DragonFlyBSD`, `Illumos`, `Solaris`, `Android`, `IOS`, `AIX`, `Haiku`, `Fuchsia`, `QNX`, `Redox` — so the BSDs are told apart. Enum variants in a `when` condition are always written in full as `Enum::Variant`, including for the program's own enums (`when Build == Mode::Fast`); the bare `.Variant` shorthand is not accepted. A variant the enum does not have is an error, and one no build target can currently produce (`OperatingSystem::Haiku`) is a warning, rather than a branch that quietly never runs. A condition may only name a Rux build intrinsic (`CurrentTarget`, `CurrentBuild`, …) or built-in enum (`OperatingSystem`, `Architecture`, …) that the file has brought into scope — `import Rux::{ CurrentTarget, OperatingSystem };` or a local `intrinsic` declaration; otherwise it is an `unknown identifier` error.
- **Compile-time build context** — importing the Rux package exposes ordinary values for the target (`CurrentTarget.os`, `.arch`, `.abi`, `.endian`, `.pointerBits`, `.dataModel`, `.objectFormat`, `.triple`, `.HasFeature(...)`), build (`CurrentBuild.profile`, `.mode`, `.optimization`, `.debugAssertions`, `.debugInfo`, `.isTest`, `.outputKind`, `.timestamp`, `.date`, `.time`), compiler (`CurrentCompiler.version`, `.HasFeature(...)`), and source location (`CurrentSource.line`, `.column`, `.file`, `.fileName`, `.filePath`, `.function`, `.module`). `CurrentConfig.Get("name")` and `CurrentConfig.Has("name")` read user-defined build values. Package authors declare these with the `intrinsic` keyword — `intrinsic const CurrentTarget: Target;` — and the compiler supplies the value. The old compiler-parameter sigils and flat aliases have been removed.
...
- **Fully-qualified item imports** — importing an item now requires its complete `Package::Module::…::Item` path; the shortcut that let a bare `import Package::Item` reach into a package's same-named module is gone. `import Memory::Alloc` is an error (with a `did you mean 'import Memory::Memory::Alloc'?` hint) — the package name is always the first segment and the containing module must be named. Items declared at a package's root (`import Rux::{ CurrentTarget }`) are unaffected.
...
- **Metadata blocks removed** — every `#{...}` declaration metadata block is now an error. Calling conventions use `#Abi(.C)`, `#Abi(.SysV)`, or `#Abi(.Win64)`; platform selection uses conditional compilation such as `when CurrentTarget.os == OperatingSystem::Windows { ... }`.
- **PascalCase compiler context and target variants** — compiler-provided values are now `CurrentTarget`, `CurrentBuild`, `CurrentCompiler`, `CurrentConfig`, and `CurrentSource`. Target enum variants use unambiguous PascalCase spellings such as `IOS`, `X86Bit64`, and `RiscvLp64`; Windows ISO code pages use names such as `Iso8859Part1`.
...
- **Target-specific dependency sections** — `Rux.toml` no longer recognizes `[Dependencies.Target.<OS>]` (or the legacy `[Target.<OS>.Dependencies]`); only a single `[Dependencies]` table is allowed. Platform selection belongs in source with conditional compilation such as `when CurrentTarget.os == OperatingSystem::Windows { ... }`.
