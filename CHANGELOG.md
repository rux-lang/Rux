...
- **Cross-target builds** — a `--target <triple>` naming something other than the host is no longer refused, and `rux run` and `rux test` accept the option that `build`, `check`, `doc`, `install`, and `update` already had. Machine code and executables are produced entirely in-process for both architectures, so a Linux host writes a Windows PE, a macOS Mach-O, or an AArch64 Linux ELF with no external toolchain and no change in output for the host triple. What a target needs is a writer for its object format rather than a machine of its own kind: x86-64 reaches every supported operating system, `linux-aarch64` reaches from any host, and `macos-aarch64`, `windows-aarch64`, and `freebsd-aarch64` are refused with a diagnostic naming the triple — on a host of that same kind too — until the AArch64 Mach-O and PE writers exist. `run` and `test` execute what they build, so they additionally require a target this host can run — directly, or under an emulator, described in the next entry.
...
- **`linux-aarch64` cross job in CI** — a third Linux job builds and runs the entire Rux suite for `linux-aarch64` on an x86-64 runner under `qemu-user`, reusing the binary the x86-64 build job already produced. Both architectures are therefore covered on every Linux run whichever runners are available.
