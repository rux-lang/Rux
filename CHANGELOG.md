...
- **Built-in assertions** — `Assert(condition, message)` is available without an import and reports the message, function, file, line, and column before trapping when its condition is false. `DebugAssert(condition, message)` has the same signature and behavior in Debug builds, while Release builds remove the check and do not evaluate its arguments.
- **Built-in panic and no-return functions** — `Panic(message)` reports the message and source context before terminating without an import. `#NoReturn()` marks user or extern functions that never return and makes calls terminate their control-flow path without adding a new type or keyword.
...
- **`#When(condition)`** — conditionally includes one declaration when its compile-time Boolean condition is true. It is shorthand for a single-branch `#if`, accepts the same constants and build intrinsics, works on imports and methods as well as top-level declarations, and removes false items before name collection and type checking.
- **Conditional compilation** — `#if cond { ... } else if cond { ... } else { ... }` evaluates its condition while compiling and keeps only the taken branch; the others are discarded before type checking, so they may reference symbols that do not exist on the current build. Conditions are built from `const` declarations, enums, literals, namespaced compile-time intrinsics such as `#target.os`, and the usual operators (`!`, `&&`, `||`, comparisons, arithmetic). It works both between declarations and inside a function body, where it introduces no scope of its own — a binding it makes is visible to the statements that follow. A condition that is not a compile-time constant, or not a `bool`, is an error.
- **`#target.os` and the `.Variant` shorthand** — `#target.os` is a compile-time value of the built-in `OS` enum, so a build selects its platform code with `#if #target.os == .Windows { ... }`. Each variant is a system, not a family: `.Windows`, `.Linux`, `.MacOS`, `.FreeBSD`, `.OpenBSD`, `.NetBSD`, `.DragonFlyBSD`, `.Illumos`, `.Solaris`, `.Android`, `.iOS`, `.AIX`, `.Haiku`, `.Fuchsia`, `.QNX`, `.Redox` — so the BSDs are told apart. An enum variant may be written without its type wherever a `#if` condition supplies it, including for the program's own enums (`#if Build == .Fast`); `OS::Windows` is the equivalent long form. A variant the enum does not have is an error, and one no build target can currently produce (`.Haiku`) is a warning, rather than a branch that quietly never runs.
- **Compile-time build context** — namespaced intrinsics expose the target (`#target.os`, `.arch`, `.abi`, `.endian`, `.pointerBits`, `.dataModel`, `.objectFormat`, `.triple`, `.hasFeature(...)`), build (`#build.profile`, `.mode`, `.optimization`, `.debugAssertions`, `.debugInfo`, `.isTest`, `.outputKind`, `.timestamp`, `.date`, `.time`), compiler (`#compiler.version`, `.hasFeature(...)`), and source location (`#source.line`, `.column`, `.file`, `.fileName`, `.filePath`, `.function`, `.module`). `#config("name")` and `#config.has("name")` read user-defined build values. Flat aliases such as `#line`, `#os`, and `#compilerVersion` have been removed.
...
- **Compile-time configuration** — `[Build.Defines]` supplies string values to `#config`, and `--define NAME[=VALUE]` overrides them for `build`, `check`, `run`, and `test`. Date/time intrinsics share one UTC build timestamp and honor `SOURCE_DATE_EPOCH` for reproducible builds.
...
- **Metadata blocks removed** — every `#{...}` declaration metadata block is now an error. Calling conventions use `#Abi(.C)`, `#Abi(.SysV)`, or `#Abi(.Win64)`; platform selection uses conditional compilation such as `#if #target.os == .Windows { ... }`.
