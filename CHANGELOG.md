...
- **Conditional compilation** — `#if cond { ... } else if cond { ... } else { ... }` evaluates its condition while compiling and keeps only the taken branch; the others are discarded before type checking, so they may reference symbols that do not exist on the current build. Conditions are built from `const` declarations, enums, literals, imported compiler parameters such as `target.os`, and the usual operators (`!`, `&&`, `||`, comparisons, arithmetic). It works both between declarations and inside a function body, where it introduces no scope of its own — a binding it makes is visible to the statements that follow. A condition that is not a compile-time constant, or not a `bool`, is an error.
- **`target.os` and the `OperatingSystem` enum** — `target.os` is a compile-time value of the built-in `OperatingSystem` enum, so a build selects its platform code with `#if target.os == OperatingSystem::Windows { ... }`. Each variant is a system, not a family: `Windows`, `Linux`, `MacOS`, `FreeBSD`, `OpenBSD`, `NetBSD`, `DragonFlyBSD`, `Illumos`, `Solaris`, `Android`, `iOS`, `AIX`, `Haiku`, `Fuchsia`, `QNX`, `Redox` — so the BSDs are told apart. Enum variants in a `#if` condition are always written in full as `Enum::Variant`, including for the program's own enums (`#if Build == Mode::Fast`); the bare `.Variant` shorthand is not accepted. A variant the enum does not have is an error, and one no build target can currently produce (`OperatingSystem::Haiku`) is a warning, rather than a branch that quietly never runs. A condition may only name a Rux build intrinsic (`target`, `build`, …) or built-in enum (`OperatingSystem`, `Architecture`, …) that the file has brought into scope — `import Rux::{ target, OperatingSystem };` or a local `#Intrinsic` declaration; otherwise it is an `unknown identifier` error.
- **Compile-time build context** — importing the Rux package exposes ordinary values for the target (`target.os`, `.arch`, `.abi`, `.endian`, `.pointerBits`, `.dataModel`, `.objectFormat`, `.triple`, `.hasFeature(...)`), build (`build.profile`, `.mode`, `.optimization`, `.debugAssertions`, `.debugInfo`, `.isTest`, `.outputKind`, `.timestamp`, `.date`, `.time`), compiler (`compiler.version`, `.hasFeature(...)`), and source location (`source.line`, `.column`, `.file`, `.fileName`, `.filePath`, `.function`, `.module`). `config.get("name")` and `config.has("name")` read user-defined build values. Package authors declare these with `#Intrinsic("Target") const $target: Target;`: `$` marks compiler initialization and is not part of the imported name. The old compiler-parameter sigils and flat aliases have been removed.
...
- **Target-specific dependency sections** — `Rux.toml` no longer recognizes `[Dependencies.Target.<OS>]` (or the legacy `[Target.<OS>.Dependencies]`); only a single `[Dependencies]` table is allowed. Platform selection belongs in source with conditional compilation such as `#if target.os == .Windows { ... }`.
