...
- **Conditional compilation** — `#if cond { ... } else if cond { ... } else { ... }` evaluates its condition while compiling and keeps only the taken branch; the others are discarded before type checking, so they may reference symbols that do not exist on the current build. Conditions are built from `const` declarations, enums, literals, `#os`, and the usual operators (`!`, `&&`, `||`, comparisons, arithmetic). It works both between declarations and inside a function body, where it introduces no scope of its own — a binding it makes is visible to the statements that follow. A condition that is not a compile-time constant, or not a `bool`, is an error.
- **`#os` and the `.Variant` shorthand** — `#os` is a compile-time value of the built-in `OS` enum, so a build selects its platform code with `#if #os == .Windows { ... }`. Each variant is a system, not a family: `.Windows`, `.Linux`, `.MacOS`, `.FreeBSD`, `.OpenBSD`, `.NetBSD`, `.DragonFlyBSD`, `.Illumos`, `.Solaris`, `.Android`, `.iOS`, `.AIX`, `.Haiku`, `.Fuchsia`, `.QNX`, `.Redox` — so the BSDs are told apart. An enum variant may be written without its type wherever a `#if` condition supplies it, including for the program's own enums (`#if Build == .Fast`); `OS::Windows` is the equivalent long form. A variant the enum does not have is an error, and one no build target can currently produce (`.Haiku`) is a warning, rather than a branch that quietly never runs.
- **Compile-time build context** — namespaced intrinsics expose the target (`#target.os`, `.arch`, `.abi`, `.endian`, `.pointerBits`, `.dataModel`, `.objectFormat`, `.triple`, `.hasFeature(...)`), build (`#build.profile`, `.mode`, `.optimization`, `.debugAssertions`, `.debugInfo`, `.isTest`, `.outputKind`, `.timestamp`, `.date`, `.time`), compiler (`#compiler.version`, `.hasFeature(...)`), and source location (`#source.line`, `.column`, `.file`, `.fileName`, `.filePath`, `.function`, `.module`). `#config("name")` and `#config.has("name")` read user-defined build values. Existing concise spellings such as `#line`, `#date`, `#ruxVersion`, and `#os` remain available.
...
- **`#{ target: "..." }` removed** — conditional compilation replaces it. The metadata key said "this declaration belongs to that OS family"; `#if #os == .Windows { ... }` says the same thing, and also works for statements, names a single system rather than a family, and groups the declarations that go together instead of repeating the key on each one. A `#{ target: ... }` block is now an error pointing at the replacement. Inside an `extend` body, a `#if` may wrap methods.
...
- **Attributes on extend methods** — a `#{ ... }` metadata block is now accepted on a method inside an `extend` block instead of failing with "expected 'func' in extend body" (#197).
