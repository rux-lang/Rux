...
- **Hardened CI and releases** — all workflows use current action versions, least-privilege permissions, credential-free checkouts, bounded timeouts, pinned CMake/Ninja versions, consistent x64 artifact names, and seven-day intermediate artifact retention. Every platform now runs C++ unit tests plus Rux check, lint, and test verification; release assets include SHA-256 checksums.
...
- **`intrinsic`** — marks a declaration the compiler supplies rather than Rux source: `intrinsic #target: Target;` and `intrinsic func Assert(condition: bool, message: char8[]);`. A compile-time value or function carries a `#`-prefixed name — `#target`, `#Error(message)` — which marks it as compiler-injected; ordinary runtime intrinsics (`Assert`, `Panic`) and methods keep their plain names. The declaration names the intrinsic, so nothing is written twice: a `#` value takes its type (`Target`), a free function its own name, and a method the type it extends (`extend Config { intrinsic func Get(...); }` is `Config.Get`). An intrinsic value has no initializer and an intrinsic function has no body. This replaces the `#Intrinsic("Name")` attribute and the `$` sigil, which together marked the same thing three ways and had to be kept in agreement.
- **`when` — conditional compilation** — `when cond { ... } else when cond { ... } else { ... }` evaluates its condition while compiling and keeps only the taken branch; the others are never resolved or type-checked, so they may reference symbols that do not exist on the current build. `if` is the run-time conditional and `when` the compile-time one, and a chain keeps the keyword it opened with: `else if` inside a `when` chain is an error. Conditions are built from `const` declarations, enums, literals, imported compile-time intrinsics such as `#target.os`, and the usual operators (`!`, `&&`, `||`, comparisons, arithmetic). It works both between declarations and inside a function body, where it introduces no scope of its own — a binding it makes is visible to the statements that follow. A condition that is not a compile-time constant, or not a `bool`, is an error.
- **`#target.os` and the `OperatingSystem` enum** — `#target.os` is a compile-time value of the built-in `OperatingSystem` enum, so a build selects its platform code with `when #target.os == .Windows { ... }`. Each variant is a system, not a family: `Windows`, `Linux`, `MacOS`, `FreeBSD`, `OpenBSD`, `NetBSD`, `DragonFlyBSD`, `Illumos`, `Solaris`, `Android`, `IOS`, `AIX`, `Haiku`, `Fuchsia`, `QNX`, `Redox` — so the BSDs are told apart. An enum variant in a `when` condition may be written in full as `Enum::Variant`, or as the `.Variant` shorthand when the other side of the comparison fixes the enum. A variant the enum does not have is an error, and one no build target can currently produce (`.Haiku`) is a warning, rather than a branch that quietly never runs. A condition may only name a Rux compile-time intrinsic (`#target`, `#build`, …) or built-in enum (`OperatingSystem`, `Architecture`, …) that the file has brought into scope — `import Rux::{ #target, OperatingSystem };` or a local `intrinsic` declaration; otherwise it is an `unknown identifier` error.
- **Compile-time build context** — importing the Rux package exposes ordinary values for the target (`#target.os`, `.arch`, `.abi`, `.endian`, `.pointerBits`, `.dataModel`, `.objectFormat`, `.triple`, `.HasFeature(...)`), build (`#build.profile`, `.mode`, `.optimization`, `.debugAssertions`, `.debugInfo`, `.isTest`, `.outputKind`, `.timestamp`, `.date`, `.time`), compiler (`#compiler.version`, `.HasFeature(...)`), and source location (`#source.line`, `.column`, `.file`, `.fileName`, `.filePath`, `.function`, `.module`). `#config.Get("name")` and `#config.Has("name")` read user-defined build values. Package authors declare these with the `intrinsic` keyword — `intrinsic #target: Target;` — and the compiler supplies the value. The old compiler-parameter sigils and flat aliases have been removed.
- **`#Error` and `#Warn` compile-time directives** — calling `#Error("message")` or `#Warn("message")` emits a compilation error or warning at the call site and produces no runtime code, so a build can reject an unsupported configuration or flag a deprecated path: `when #target.os == .Windows { #Error("Windows is not supported yet"); }`. Because they run after `when` folds, a directive in a branch that is not taken never fires. The message must be a string literal. They are declared as ordinary `#`-prefixed intrinsic functions (`intrinsic func #Error(message: char8[]);`) and imported from the Rux package.
- **`when` match over a compile-time value** — besides the `when <cond> { ... }` chain, `when` has a match form `when <subject> { pattern => body, ... else => body }` that evaluates the subject while compiling and keeps only the matching arm, so a build selects platform code by value: `when #target.os { .Windows => PrintLine("win"), .Linux => PrintLine("linux"), else => #Error("Unsupported OS") }`. Arm patterns are enum variants (the `.Variant` shorthand or the full `Enum::Variant`) or literals; an arm may list several comma-separated patterns and is taken when any matches (`.Windows, .Linux => ...`), and `else` is the default. An arm body is a single expression, a `{ ... }` block, or — in the declaration form — a declaration (a bare `import` needs no trailing `;`) or an `#Error`/`#Warn` directive. It works both inside a function body (arms are statements) and between declarations (arms are declarations), splicing the chosen arm with no scope of its own. If no arm matches and there is no `else`, the build is an error.
...
- **Metadata blocks removed** — every `#{...}` declaration metadata block is now an error. Calling conventions use `#Abi(.C)`, `#Abi(.SysV)`, or `#Abi(.Win64)`; platform selection uses conditional compilation such as `when #target.os == .Windows { ... }`.
- **`#`-prefixed compiler context and PascalCase target variants** — compiler-provided values are now `#target`, `#build`, `#compiler`, `#config`, and `#source`. Target enum variants use unambiguous PascalCase spellings such as `IOS`, `X86Bit64`, and `RiscvLp64`; Windows ISO code pages use names such as `Iso8859Part1`.
...
- **Target-specific dependency sections** — `Rux.toml` no longer recognizes `[Dependencies.Target.<OS>]` (or the legacy `[Target.<OS>.Dependencies]`); only a single `[Dependencies]` table is allowed. Platform selection belongs in source with conditional compilation such as `when #target.os == .Windows { ... }`.
