...
struct RegistryVersion {
    ...
    /// Yanked versions stay listed so an existing install can still be identified; a new resolution must not select
    /// one.
    bool yanked = false;
 ...
 *
 * `code` is the RFC 9457 problem code when there was one, which is stable and machine-readable; `detail` is the
 * registry's own prose. The CLI turns these into user-facing text.
 */
...
/// Decode a `GET /v1/index/{ns}/{package}` body. Exposed so the decoding is testable without a network.
...
/// `GET {base}/v1/packages/{ns}/{package}/{version}`, reduced to the artifact's lowercase-hex SHA-256.
...
/// `GET {base}/v1/packages/{ns}/{package}/{version}/download`, following the redirect the registry answers with, to the
/// complete `.ruxpkg` bytes.
 ...
 *
 * The order matters for diagnostics: a version is only reported as yanked or as needing a newer compiler when the
 * requirements would otherwise have accepted it, so an error never blames a rule the user was nowhere near.
 */
 ...
 *
 * Yanked versions are skipped, as is any version whose declared minimum compiler release is newer than `compiler`. The
 * highest remaining match wins, ordered by SemVer precedence with build metadata as the tie-break, which is the
 * registry's own total order.
 *
 ...
 *
 * Naming only the requirement would be misleading whenever a version did match it and lost to the yank or
 * compiler-minimum rule instead: the reader would be told that nothing satisfies a requirement directly above the
 * version that does. So when one excluded version accounts for the whole failure, the reason *is* the message and there
 * is nothing below it; the requirement only leads when several versions failed for different reasons, or when none
 * matched at all and the published list is the useful thing to show.
 */
...
/// Every version text in `entry`, ascending, for an error that has to say what the registry does offer.
