...
// Compiler/Platform/ so the rest of the compiler is platform-agnostic; CI
// greps for strays (see Tools/CheckPlatformIsolation.sh).
...
[[nodiscard]] std::string ExecutableFileName(std::string name, OS os = HostOS);
...
[[nodiscard]] std::string SharedLibraryFileName(std::string name, OS os = HostOS);
