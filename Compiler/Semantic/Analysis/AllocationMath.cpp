...
TypeRef AnalysisContext::CheckTypeQueryExpression(const TypeQueryExpr &expression) {
...
bool AnalysisContext::IsCheckedArithmeticIntrinsic(const std::string_view intrinsicName) {
    return std::ranges::find(kCheckedOperations, intrinsicName) != kCheckedOperations.end();
}

void AnalysisContext::ValidateCheckedArithmeticIntrinsic(const FuncDecl &declaration) {
