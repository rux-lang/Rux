...
namespace Rux {
/// The import name a package conventionally binds the intrinsics package to. Only a default: the name is a manifest's
/// choice, not the language's.
...
struct CompileTimeContext {
    ...
    /// Import names under which the file being folded has the intrinsics package in scope. A `when` condition may only
    /// name a build intrinsic or built-in enum the file imported from that package, and a manifest is free to bind it
    /// to any import name, so the spelling cannot be assumed: the driver fills this in from the resolved package
    /// identity of each dependency. The default covers embedders and focused tests that declare no manifest at all.
