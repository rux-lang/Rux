...
A per-user installer that downloads a published Rux release from GitHub, installs the `rux` binary, and makes it available from any terminal.
...
```sh
curl -fsSLO https://rux-lang.dev/install.sh
chmod +x install.sh
...
Run the installer again to replace the existing binary with the latest or a pinned release. To uninstall the default per-user installation:

```sh
rm "$HOME/.local/bin/rux"
```

If the installer added a PATH line, remove the adjacent `Added by the Rux installer` entry from the shell rc file named during installation.
