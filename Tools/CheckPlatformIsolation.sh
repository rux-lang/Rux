...
# environment access, ...) must stay inside Compiler/Platform/ so the rest of
...
# these facilities, add or reuse a wrapper in Platform/ (see Platform/Os.h and
# Platform/Process.h); if the Platform layer grows a new wrapper, extend the
...
pattern='<windows\.h>|Platform/WinApi\.h'
...
violations=$(grep -rnE "$pattern" Compiler --include='*.cpp' --include='*.h' | grep -v '^Compiler/Platform/' || true)

if [ -n "$violations" ]; then
    echo "error: direct OS API usage outside Compiler/Platform/ — route it through the Platform layer:" >&2
    echo "$violations" >&2
    exit 1
fi
