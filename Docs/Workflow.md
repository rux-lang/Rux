...
`sh Run.sh unit` and `./Run.ps1 unit` run the same CTest invocation against an existing build and report it in the `rux test` vocabulary: `Running <n> tests`, one `Passed <group> in <duration>` or `Failed <group> in <duration>` line per CTest group as it completes, and a `<n> tests in <duration> (<passed> passed, <failed> failed)` total; a failing group's own output passes through unchanged. The unit-test executable is written to `Bin/Tests/Unit/rux-tests` (`rux-tests.exe` on Windows); CTest resolves that path automatically.
