...
For a foreign compiler host, a Release cross-build adds the canonical target below the configured output directory. With the default `Output = "Bin"`, the three package kinds therefore write:
...
A native `macos-aarch64` compiler keeps its historical host path and omits the target component. Cross-produced artifacts are not launched by Rux. Transfer an executable or its libraries to an Apple Silicon Mac, preserve executable permissions, and launch it there, for example:
