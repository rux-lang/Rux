...
Semantic declaration collection is owned by the private `SemanticProgramIndex`. It builds package-isolated root scopes, logical module scopes, overload sets, and declaration-to-source/module ownership before signature resolution and checking begin. The analyzer supplies in-progress type resolution for typed constants and aliases, consumes the index for imports and visibility, and remains the sole owner of semantic checking and diagnostic policy. The index accumulates duplicate-definition diagnostics through the semantic result and never renders or prints them.
