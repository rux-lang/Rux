...
Which triples a back end reaches is decided by the object and image writer, not by the host: x86-64 reaches every supported operating system, while AArch64 reaches `linux-aarch64` through ELF, `windows-aarch64` through PE/COFF, and `macos-aarch64` through Mach-O. The Windows writer produces executables, DLLs with import libraries, and static libraries entirely in-process. The Mach-O path likewise produces AArch64 objects and BSD archives plus signed freestanding executables, imported executables, and shared libraries. The driver continues to refuse `macos-aarch64` until its end-to-end artifact coverage enables the public target. Other AArch64 targets without an image writer, including FreeBSD, are refused with a diagnostic naming the target rather than falling back to a host toolchain.
