...
AST-to-HIR implementation files share private state through `AstToHirDetail::AstToHirContext`. It owns module and declaration ordering, lowering scopes, statement/pattern construction, basic expressions, aggregate/compiler-intrinsic expressions and aggregate coercions from checked semantic facts, and call/dispatch lowering from recorded callable bindings and symbol identities, while the public `AstToHirLowering` API remains the stage boundary. Aggregate lowering is the single owner for paths, ranges, arrays, slices, tuples, structs, enums and match expressions; contextual coercion preserves their recorded semantic types and interface layout identities. The residual `AstToHir.cpp` remains temporarily above the 1,200-line guideline because type resolution and declaration lowering still share the concrete lowering implementation; RUX-027E tracks that final cohesive extraction instead of obscuring the boundary with a mechanical split.
