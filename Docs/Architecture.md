...
AST-to-HIR implementation files share private state through `AstToHirDetail::AstToHirContext`. It owns module and declaration ordering, lowering scopes, statement/pattern construction, basic and aggregate/compiler-intrinsic expression lowering from checked semantic facts, and call/dispatch lowering from recorded callable bindings and symbol identities, while the public `AstToHirLowering` API remains the stage boundary. The residual `AstToHir.cpp` remains temporarily above the 1,200-line guideline because type resolution, declaration lowering, coercion and residual expression dispatch still share the concrete lowering implementation; RUX-027D and RUX-027E track those cohesive extractions instead of obscuring the boundaries with a mechanical split.
