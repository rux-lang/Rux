...
AST-to-HIR implementation files share private state through `AstToHirDetail::AstToHirContext`. It owns module and declaration ordering, lowering scopes, statement/pattern construction, basic expression lowering from checked semantic type facts, and call/dispatch lowering from recorded callable bindings and symbol identities, while the public `AstToHirLowering` API remains the stage boundary.
