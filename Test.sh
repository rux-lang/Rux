...
if [ "$clang_tidy_enabled" = true ]; then
    ...
    [ -n "$run_clang_tidy" ] || die "required tool 'run-clang-tidy' was not found; install it and ensure it is available on PATH"
fi
...
if [ "$skip_build" = false ]; then
    if [ -n "$compiler" ]; then
        run_checked Build.sh sh "$repository_root/Build.sh" \
...
if [ "$fix_formatting" = true ]; then
    run_checked Format.sh sh "$repository_root/Format.sh" --rux-executable "$rux_path"
else
    run_checked Format.sh sh "$repository_root/Format.sh" --check --rux-executable "$rux_path"
fi

if [ "$clang_tidy_enabled" = true ]; then
    compile_commands=$build_path/compile_commands.json
    [ -f "$compile_commands" ] || die "compilation database '$compile_commands' was not found; run Test.sh without --skip-build first"
    ...
    step "Running clang-tidy ($clang_tidy_files files)"
    ...
    if "$run_clang_tidy" -quiet -j "$clang_tidy_jobs" \
        ...
        if [ -t 1 ] && [ -z "${NO_COLOR:-}" ]; then
            printf '\033[32m[PASSED]\033[0m clang-tidy (%s files) in %s\n' "$clang_tidy_files" "$(format_duration "$clang_tidy_elapsed")"
        else
            printf '[PASSED] clang-tidy (%s files) in %s\n' "$clang_tidy_files" "$(format_duration "$clang_tidy_elapsed")"
        fi
    else
        clang_tidy_elapsed=$(( $(date +%s) - clang_tidy_started_at ))
        if [ -t 1 ] && [ -z "${NO_COLOR:-}" ]; then
            printf '\033[31m[FAILED]\033[0m clang-tidy in %s\n' "$(format_duration "$clang_tidy_elapsed")"
        else
            printf '[FAILED] clang-tidy in %s\n' "$(format_duration "$clang_tidy_elapsed")"
        fi
        exit 1
    fi
fi
